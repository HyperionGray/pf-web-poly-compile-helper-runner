...
if "$REPO_ROOT/install.sh" --prefix "$NATIVE_PREFIX" --skip-deps >"$NATIVE_LOG" 2>&1; then
    ...
    # Test task listing
    if cd "$REPO_ROOT" && "$NATIVE_PREFIX/bin/pf" -f "$FIXTURE_PFYFILE" list >/dev/null 2>&1; then
    ...
    # Test task execution
    if cd "$REPO_ROOT" && "$NATIVE_PREFIX/bin/pf" -f "$FIXTURE_PFYFILE" run hello >/dev/null 2>&1; then
...
if "$REPO_ROOT/install-static.sh" --prefix "$STATIC_PREFIX" >/dev/null 2>&1; then
    ...
    # Test task listing
    if cd "$REPO_ROOT" && "$STATIC_PREFIX/bin/pf" -f "$FIXTURE_PFYFILE" list >/dev/null 2>&1; then
    ...
    # Test task execution
    if cd "$REPO_ROOT" && "$STATIC_PREFIX/bin/pf" -f "$FIXTURE_PFYFILE" run hello >/dev/null 2>&1; then
        log_success "Static pf task execution works"
    else
        log_error "Static pf task execution failed"
    fi
else
...
if [[ $TESTS_FAILED -eq 0 ]]; then
    ...
    echo "  ✓ install-static.sh (source-based installer)"
    echo ""
