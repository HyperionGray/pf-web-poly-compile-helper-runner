...
class DocumentationValidator {
    constructor(rootPath = path.resolve(__dirname, '../..')) {
        ...
        this.requiredRootDocs = [
            // Keep documentation physically under docs/.
            'docs/QUICKSTART.md',
            'docs/CHANGELOG.md',
            'docs/CONTRIBUTING.md',
            'docs/CODE_OF_CONDUCT.md',
            'docs/SECURITY.md',
            // License is kept at repo root.
            'LICENSE',
        ];
        this.requiredDocsStructure = {
            'docs/INSTALLER_GUIDE.md': 'installation guide',
            'docs/DEVELOPER_NAVIGATION_GUIDE.md': 'documentation index / navigation',
        };
        this.readmeLikeDocCandidates = [
            'docs/DEVELOPER_NAVIGATION_GUIDE.md',
            'docs/QUICKSTART.md',
        ];
    ...
    async validateDocumentationStructure() {
        ...
        // Analyze primary guide content (README-like doc under docs/)
        console.log('📖 Analyzing primary guide content...');
        const primaryDocRel = this.readmeLikeDocCandidates.find((p) => fs.existsSync(path.join(this.rootPath, p)));
        const primaryDocPath = primaryDocRel ? path.join(this.rootPath, primaryDocRel) : null;
        if (primaryDocPath) {
            results.readmeAnalysis = await this.analyzeReadmeContent(primaryDocPath);
        } else {
    ...
    generateRecommendations(results, additionalDocs) {
        ...
        if (results.readmeAnalysis.missingSection && results.readmeAnalysis.missingSection.length > 0) {
            recommendations.push({
                ...
                priority: 'medium',
                message: `Primary guide missing ${results.readmeAnalysis.missingSection.length} required sections`,
    ...
    formatForCICD(validationResults, additionalDocs = []) {
        ...
        output += `- **Guide Completeness**: ${report.summary.readmeCompleteness}% (${report.readmeAnalysis.requiredSections - (report.readmeAnalysis.missingSection?.length || 0)}/${report.readmeAnalysis.requiredSections} required sections)\n`;
