...
cd "$REPO_ROOT"
TASK_LIST=$($PF_CMD pf-files/Pfyfile.pf list 2>&1)
...
TASK_DEF=$(grep -A5 "task web-build-rust-llvm" "$REPO_ROOT/pf-files/Pfyfile.pf" 2>/dev/null || echo "")
...
TASK_DEF=$(grep -A5 "task web-build-c-llvm" "$REPO_ROOT/pf-files/Pfyfile.pf" 2>/dev/null || echo "")
...
# Test 12.2: install-all exists
if echo "$TASK_LIST" | grep -q "install-all"; then
    pass "install-all task exists"
else
    fail "install-all task exists" "Task not found"
fi
