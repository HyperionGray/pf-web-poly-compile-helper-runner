...
11. **Core pf-files/Pfyfile.pf has valid syntax** - Main configuration file is correct
...
1. **Web Development** (`pf-files/web-testing/Pfyfile.web.pf`)
   - `web-build-*` - Build tasks for Rust, C, Fortran, WAT
   - `web-dev` - Development server with REST API
   - `web-test` - Playwright testing

2. **Containers & Quadlets** (`pf-files/containers/Pfyfile.containers.pf`)
   - `container-build-*` - Container image building
   - `compose-*` - Docker/Podman compose operations
   - `quadlet-*` - Systemd integration

3. **Security & Exploitation** (`pf-files/vuln-hunting/Pfyfile.security.pf`, `pf-files/exploit-writing/Pfyfile.exploit.pf`)
   - `security-scan` - Web security scanning
   - `rop-*` - ROP exploitation demos
   - `heap-*` - Heap exploitation

4. **Fuzzing & Sanitizers** (`pf-files/vuln-hunting/Pfyfile.fuzzing.pf`)
   - `build-with-asan/msan/ubsan/tsan` - Sanitizer builds
   - `afl-fuzz` - AFL++ fuzzing
   - `libfuzzer-*` - libfuzzer integration

5. **Debugging & RE** (`pf-files/debugging/Pfyfile.debugging.pf`, `pf-files/debugging/Pfyfile.debug-tools.pf`)
   - `debug` - Interactive debugging
   - `install-*` - RE tool installation (Ghidra, Radare2, etc.)
   - `binary-info` - Binary analysis

6. **Binary Lifting** (`pf-files/llvm-lifting/Pfyfile.lifting.pf`)
   - `lift-binary-*` - Binary to LLVM IR
   - `optimize-lifted-ir` - LLVM optimization

7. **Binary Injection** (`pf-files/vuln-hunting/Pfyfile.injection.pf`)
   - `inject-*` - Code injection techniques
   - `create-injection-payload-*` - Payload creation

8. **Kernel Debugging** (`pf-files/debugging-deep/Pfyfile.kernel-debug.pf`)
   - `kernel-*` - Kernel-level debugging
   - `kernel-automagic-analysis` - Automated vulnerability discovery

9. **Package Management** (`pf-files/distro-switching/Pfyfile.package-manager.pf`)
   - `pkg-convert` - Cross-format package translation
   - `pkg-*` - Package operations

10. **OS & Distro Management** (`pf-files/distro-switching/Pfyfile.distro-switch.pf`, `pf-files/distro-switching/Pfyfile.os-containers.pf`)
    - `distro-install-*` - Multi-distro package installation
    - `switch-os` - OS switching (DANGEROUS)

11. **Git Tools** (`pf-files/gitops/Pfyfile.git-cleanup.pf`)
    - `git-cleanup` - Interactive TUI for history cleanup

12. **TUI** (`pf-files/always-available/Pfyfile.tui.pf`)
    - `tui` - Interactive terminal UI

13. **PR Management** (`pf-files/gitops/Pfyfile.pr-management.pf`)
    - `pr-*` - Pull request automation

14. **Smart Workflows** (`pf-files/enhanced-tasks/Pfyfile.smart-workflows.pf`)
...
**Location**: `pf-files/debugging-deep/Pfyfile.kernel-debug.pf`
...
**Location**: `pf-files/llvm-lifting/Pfyfile.lifting.pf` + `pf-files/vuln-hunting/Pfyfile.fuzzing.pf`
...
**Location**: `pf-files/distro-switching/Pfyfile.distro-switch.pf`
...
**Location**: `pf-files/always-available/Pfyfile.tui.pf`
...
**Location**: `pf-files/enhanced-tasks/Pfyfile.smart-workflows.pf`, `pf-files/enhanced-tasks/Pfyfile.enhanced-integration.pf`
...
**Location**: `pf-files/auto-api/Pfyfile.rest-api.pf`
...
### 10. **Git Cleanup TUI** ⭐⭐⭐
**Location**: `pf-files/gitops/Pfyfile.git-cleanup.pf`
...
**Found**: pf list command displays 4 tasks twice (api-server, debug-check-podman, install, sync-demo) even though they're only defined once in pf-files/Pfyfile.pf.
