...
def test_pfyfiles_included():
    ...
    root_file = "Pfyfile.pf"
    if not os.path.exists(root_file):
        print(f"❌ Main Pfyfile not found: {root_file}")
        return False

    with open(root_file, "r", encoding="utf-8") as f:
        root_content = f.read()

    if "pf/Pfyfile.pf" not in root_content:
        print("❌ Root Pfyfile.pf does not include pf/Pfyfile.pf")
        return False

    enhanced_file = "pf/Pfyfile.pf"
    if not os.path.exists(enhanced_file):
        print(f"❌ Aggregator Pfyfile not found: {enhanced_file}")
        return False
    ...
    for include in required_includes:
        if include in enhanced_content:
...
def main():
    ...
    if passed == total:
        ...
        print("   - Native install: sudo ./install.sh")
