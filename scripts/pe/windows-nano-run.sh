...
IMAGE="localhost/pf-pe-windows-nano:latest"
WORKDIR="${PWD}"

pe_require_file "${PE_PATH}" "Usage: windows-nano-run.sh /path/to/file.exe" "PE file"
pe_require_image "${RUNTIME}" "${IMAGE}" "pf pe build-windows-nano"
...
exec "${RUNTIME}" run --rm --pull=never \
  ...
  "${IMAGE}" \
  pf-execute-pe-nano "/workspace/pe-input/${PE_FILE}"
