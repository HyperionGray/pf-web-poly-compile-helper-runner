...
create_source_tarball() {
    ...
    # Copy source files (exclude build artifacts, deps, and git)
    rsync -av \
        --exclude='.git*' \
        --exclude='build/' \
...
build_deb() {
    ...
    if ! command -v dpkg-deb >/dev/null 2>&1; then
        log_error "dpkg-deb not found. Install with: sudo apt-get install dpkg"
        return 1
    fi
    ...
    # Build package using the repo's simple deb builder
    log_info "Running debian/build-deb.sh..."
    (cd debian && chmod +x build-deb.sh && ./build-deb.sh "$VERSION")

    # Move package to build directory
    mv "debian/build/pf-runner_${VERSION}.deb" "$deb_dir/"
