...
def analyze_novel_features():
    ...
    # Scan all Pfyfiles for feature indicators (root + pf tree)
    pfyfiles = list(glob.glob("Pfyfile*.pf"))
    pfyfiles.extend(str(p) for p in Path("pf").rglob("Pfyfile*.pf"))
