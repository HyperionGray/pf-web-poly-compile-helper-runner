...
- Python 3.8+, Git, and rsync

### Optional Prerequisites
- Docker or Podman **only if** you want to run the optional dev containers (`pf dev-env*` tasks)
...
./install.sh                  # system prefix if run as root, ~/.local otherwise
# optional:
# ./install.sh --prefix ~/.local --skip-deps
```

The installer is **native-only**. Container installation paths and flags (`--mode`, `--runtime`, etc.) are deprecated. Use the optional container tasks (`pf dev-env*`) only after the native install if you need them.
...
Container workflows remain available for specific tasks, but they are **not** part of the base install. After running `./install.sh`, you can bootstrap the dev containers (Podman/Docker) with:

```bash
scripts/manage-containers.sh start    # build images + start stack (container workflows optional)
scripts/manage-containers.sh status   # check container status
```
...
**Containers (optional):**
- Built and managed only when you run `scripts/manage-containers.sh` (container installer deprecated)
