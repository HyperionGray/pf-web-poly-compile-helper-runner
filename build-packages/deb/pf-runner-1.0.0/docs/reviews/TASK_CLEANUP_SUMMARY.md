...
3. **web-build-*** tasks (Pfyfile.pf vs pf-runner/pf-files/web/Pfyfile.web-demo.pf)
   - Main Pfyfile.pf has comprehensive versions for general use
   - pf-runner/pf-files/web/Pfyfile.web-demo.pf has simpler versions for pf-runner subdirectory
...
2. **pf-runner subdirectory tasks** (~15 tasks)
   - Tasks in pf-runner/pf-files/**/Pfyfile.*.pf are only loaded when running from pf-runner directory
...
2. **Documentation** - Maintain clear separation between:
   ...
   - Test files (test_*.pf)
   - Subdirectory tasks (pf-runner/pf-files/**/Pfyfile.*.pf)
...
7. `/pf-runner/pf-files/Pfyfile.pf` - Added 2 includes, excluded 1 to prevent duplicates
