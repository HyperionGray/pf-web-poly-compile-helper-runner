...
- Project split: `include` other `.pf` files from `pf-files/core/Pfyfile.pf` (recommended) or `Pfyfile.pf` (legacy fallback)
...
**See [SUBCOMMANDS.md](SUBCOMMANDS.md) for complete guide to organizing tasks with subcommands!**
...
3. **Global/file-level**: Add `#!lang:python` as the first line of your Pfyfile (e.g. `pf-files/core/Pfyfile.pf`)
...
Top-level in `pf-files/core/Pfyfile.pf`:
...
- `pf-files/core/Pfyfile.pf` - Main configuration with includes for all task categories
- `pf-files/dev/Pfyfile.dev.pf` - Development tasks (setup, lint, test, symlink)
