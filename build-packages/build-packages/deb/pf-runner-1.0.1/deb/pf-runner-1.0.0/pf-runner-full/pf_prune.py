...
class PfSyntaxChecker:
    ...
    def _get_error_suggestion(self, line: str, error_type: str) -> Optional[str]:
        ...
        # Invalid operator (only meaningful for DSL conditionals)
    ...
    def validate_syntax(self, source: str, source_file: Optional[str] = None) -> ValidationResult:
        ...
        # Track embedded shell regions so we don't parse bash as pf DSL.
        in_shell_pipe_block = False
        shell_pipe_indent = 0
        in_shell_heredoc = False
        ...
        for i, line in enumerate(lines, 1):
            ...
            # Skip content inside heredocs (either from `shell <<EOF` or within `shell |`).
            if in_shell_heredoc:
                if stripped == (shell_heredoc_delim or ""):
                    in_shell_heredoc = False
                    shell_heredoc_delim = None
                continue

            # Skip content inside `shell |` blocks (opaque).
            if in_shell_pipe_block:
                if not stripped:
                    continue
                m = script_heredoc_re.search(stripped)
                if m:
                    in_shell_heredoc = True
                    shell_heredoc_delim = m.group(2)
                    continue
                if indent > shell_pipe_indent:
                    continue
                in_shell_pipe_block = False

            # Skip content inside backslash-continued single `shell` lines.
            if in_shell_backslash_continuation:
            ...
            # Enter multiline shell modes (only meaningful inside a task).
            if current_task:
                if stripped == 'shell |':
                    in_shell_pipe_block = True
                    shell_pipe_indent = indent
                    continue
                m = shell_heredoc_re.match(stripped)
                if m:
                    in_shell_heredoc = True
                    shell_heredoc_delim = m.group(2)
                    continue
                if stripped.startswith("shell ") and line.rstrip().endswith("\\"):
                    in_shell_backslash_continuation = True
...
def prune_tasks(
    ...
    # Prefer the real parser: it understands heredocs / multiline `shell` blocks
    # and avoids false positives from heuristic scanning.
    try:
        tasks = parse_pfyfile_text(dsl_src, task_sources)
    except PFSyntaxException as e:
        lines = dsl_src.splitlines()
        line_num = int(e.line_number or 1)
        line_num = max(1, min(line_num, len(lines) or 1))
        line_content = lines[line_num - 1].rstrip() if lines else ""
        ctx_before, ctx_after = checker._get_context_lines(lines, line_num)
        ...
        err = SyntaxError(
            ...
            context_before=ctx_before,
            context_after=ctx_after,
            traceback=e._traceback if (debug_mode or verbose) else None,
        )
        print(f"\n{err.format(verbose=debug_mode or verbose)}")

        try:
            with open(output_file, "w", encoding="utf-8") as f:
                f.write("# pf prune - Failed syntax validation\n")
                f.write("# Fix the errors below and re-run pf prune\n\n")
                f.write(format_exception_for_user(e))
                f.write("\n")
            print(f"\nFailed tasks written to: {output_file}")
        except Exception as write_err:
            print(f"Warning: Could not write to {output_file}: {write_err}", file=sys.stderr)

        failed_tasks = [e.task_name] if e.task_name else []
        return 0, 1, failed_tasks
    except Exception:
        # Last-resort fallback: heuristic validation (may produce false positives).
        ...
        if not result.is_valid:
            ...
            failed_task_names = {err.task_name for err in result.errors if err.task_name}
            try:
                with open(output_file, "w", encoding="utf-8") as f:
                    f.write("# pf prune - Failed syntax validation\n")
                    f.write("# Fix the errors below and re-run pf prune\n\n")
...
def main(argv: List[str]) -> int:
    ...
    parser.add_argument(
        '-f', '--file',
        help='Pfyfile to check (default: pf-files/Pfyfile.pf, fallback: Pfyfile.pf)'
    )
    parser.add_argument(
        '-d', '--dry-run',
        action='store_true',
