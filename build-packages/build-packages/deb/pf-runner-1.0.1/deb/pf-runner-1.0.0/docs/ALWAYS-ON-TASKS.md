...
Always-on tasks are stored under `pf-files/always-available/` (notably `pf-files/always-available/Pfyfile.always-on.pf`). These tasks are intended to be available from any directory on the system.
...
1. Create a new `Pfyfile.<category>.pf` under `pf-files/` (or extend `pf-files/always-available/Pfyfile.always-on.pf`)
2. Add tasks that work independently of project context
3. Include a help task (e.g., `<category>-help`)
4. Update `pf-files/Pfyfile.pf` (or `pf-files/always-available/Pfyfile.always-available.pf`) to include the new file, as appropriate
...
2. Check that `pf-files/always-available/Pfyfile.always-on.pf` exists
3. Verify `pf-files/Pfyfile.pf` includes the files you expect (or pass `--file` explicitly)
