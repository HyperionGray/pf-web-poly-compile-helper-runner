...
- **Python** (`setup.py`, `pyproject.toml`) → `python -m build` (avoid pip/editable installs)
...
### Debian/Ubuntu Package Installation (Supported)

Install the packaged build (vendored dependencies, no pip/venv):

```bash
sudo dpkg -i build-packages/deb/pf-runner_1.0.0.deb
# or if using the debian/ build output:
...
Deprecated. Use the packaged `.deb` instead of pip/editable installs.
...
```
pf-web-poly-compile-helper-runner/
├── pf-files/                       # Categorized Pfyfiles (task definitions)
│   ├── Pfyfile.pf                  # Main pf configuration (includes categories)
│   ├── always-available/           # Always-available tasks (loaded automatically)
│   └── ...                         # Other task categories (debugging, web, containers, etc.)
├── start.sh                        # Quick setup script
│
├── pf-runner/                      # pf task runner implementation
│   ├── pf_main.py                  # CLI entrypoint
│   ├── pf_parser.py                # DSL parser + executor
│   ├── README.md                   # pf-runner documentation
...
The pf runner supports inline code execution in multiple languages. Create a `pf-files/Pfyfile.pf`:
...
#### Fabric import error
- Reinstall via the .deb (vendor deps included): `sudo dpkg -i build-packages/deb/pf-runner_1.0.0.deb`
...
- Review example tasks in `pf-files/Pfyfile.pf` and included `pf-files/**/Pfyfile*.pf` files
