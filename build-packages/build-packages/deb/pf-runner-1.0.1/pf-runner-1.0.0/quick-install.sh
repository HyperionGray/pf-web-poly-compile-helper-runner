...
# Automatically installs via .deb (debian) or native; container mode deprecated
#
...
main() {
    ...
    # If we're in the repo, use the local installer (.deb preferred)
    if in_repo; then
        ...
        if [[ "$os_type" == "debian" ]] && [[ -f "debian/build/pf-runner_${PF_VERSION}.deb" ]]; then
            ...
            log_info "Using standard native installer"
            if [[ $EUID -eq 0 ]]; then
                ./install.sh
            else
        ...
        log_info "Repository cloned - running installer (native/.deb only)"
