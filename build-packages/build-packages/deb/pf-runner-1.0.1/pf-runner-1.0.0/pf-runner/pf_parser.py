...
def _find_pfyfile(
    ...
    while True:
        ...
        if os.sep not in pf_hint and "/" not in pf_hint and "\\" not in pf_hint:
            pf_files_dir = os.path.join(cur, "pf-files")
            pf_files_direct = os.path.join(pf_files_dir, pf_hint)
            if os.path.isfile(pf_files_direct):
                return pf_files_direct
            if os.path.isdir(pf_files_dir):
                for root, _dirs, files in os.walk(pf_files_dir):
                    if pf_hint in files:
                        return os.path.join(root, pf_hint)
...
_PF_FILES_INDEX: Dict[str, Dict[str, str]] = {}


def _find_pf_files_dir(start_dir: str) -> Optional[str]:
    cur = os.path.abspath(start_dir or os.getcwd())
    if os.path.basename(cur) == "pf-files" and os.path.isdir(cur):
        return cur
    while True:
...
def _build_pf_files_index(pf_files_dir: str) -> Dict[str, str]:
    # Map filename -> best match within pf-files (prefer shallow paths, then lexicographic).
    candidates: Dict[str, List[str]] = {}
    for root, _dirs, files in os.walk(pf_files_dir):
        for name in files:
            if not name.endswith(".pf"):
                continue
            candidates.setdefault(name, []).append(os.path.join(root, name))

    index: Dict[str, str] = {}
    for name, paths in candidates.items():
        paths_sorted = sorted(paths, key=lambda p: (p.count(os.sep), p))
        index[name] = os.path.normpath(paths_sorted[0])
    return index


def _resolve_include_path(inc_path: str, base_dir: str) -> Optional[str]:
    if os.path.isabs(inc_path):
        resolved = os.path.normpath(inc_path)
        return resolved if os.path.exists(resolved) else None

    # First: relative to the including file's directory (existing behavior).
    direct = os.path.normpath(os.path.join(base_dir, inc_path))
    if os.path.exists(direct):
        return direct

    # Next: relative to pf-files root (supports categorized Pfyfiles).
    pf_files_dir = _find_pf_files_dir(base_dir)
    if not pf_files_dir:
        return None

    from_pf_files = os.path.normpath(os.path.join(pf_files_dir, inc_path))
    if os.path.exists(from_pf_files):
        return from_pf_files

    # Finally: if the include is a bare filename, search pf-files recursively.
    if os.path.basename(inc_path) != inc_path:
        return None

    index = _PF_FILES_INDEX.get(pf_files_dir)
    if index is None:
        index = _build_pf_files_index(pf_files_dir)
        _PF_FILES_INDEX[pf_files_dir] = index
    return index.get(inc_path)
...
def _expand_includes_from_text(
    ...
    for raw in text.splitlines():
        ...
        if not inside_task and stripped.startswith("include "):
            ...
            if len(toks) >= 2:
                inc_path = toks[1]
                inc_full = _resolve_include_path(inc_path, base_dir)
                if not inc_full:
                    print(
                        f"[warn] include file not found: {os.path.normpath(os.path.join(base_dir, inc_path))}",
                        file=sys.stderr,
                    )
                    continue
                inc_full = os.path.abspath(inc_full)
...
def _load_pfy_source_with_includes(
    ...
    repo_root_guess = os.path.dirname(script_dir)

    always_available_candidates = [
        # Alternate categorized layout: pf-files/always/
        os.path.join(
            repo_root_guess, "pf-files", "always", "Pfyfile.always-available.pf"
        ),
        # Preferred (repo layout): pf-files/always-available/
        os.path.join(
            repo_root_guess, "pf-files", "always-available", "Pfyfile.always-available.pf"
        ),
        # Legacy (repo layout): repository root
        os.path.join(repo_root_guess, "Pfyfile.always-available.pf"),
        # Installed layout: alongside the module (if packaged that way)
        os.path.join(script_dir, "Pfyfile.always-available.pf"),
    ]
...
def parse_pfyfile(file_path: str) -> Dict[str, Task]:
    """Parse a Pfyfile from disk (without implicit always-available tasks).

    This is primarily a compatibility wrapper for tests and tooling.
    """
    ...
    expanded, task_sources = _expand_includes_from_text(text, base_dir, visited, pfy_abs)
...
def _alias_map(names: List[str]) -> Dict[str, str]:
    """Build a mapping of common name variants to their canonical form.

    This is intentionally conservative: it supports hyphen/underscore swaps and
    case-insensitive matches, without inventing new aliases for user tasks.
    """
    canonical = set(names)
    out: Dict[str, str] = {}
    for name in names:
        ...
        for v in variants:
            if v and v != name and v not in canonical and v not in out:
                out[v] = name
    return out
...
def _c_for(
    ...
    if not _FABRIC_AVAILABLE:
        raise PFEnvironmentError(
            message="Fabric is required for remote execution (non-@local hosts).",
            suggestion='Install with: pip install "fabric>=3.2,<4"',
        )
...
def _exec_line_fabric(
    ...
    if connection is None:
        ...
        import subprocess
        try:
            shell_exe = "/bin/bash" if os.path.exists("/bin/bash") else None
...
def main(argv: List[str]) -> int:
    ...
    while i < len(tokens):
        ...
        # First non-global token marks the start of task execution. Do not
        # interpret subsequent `host=...`, `port=...`, etc. as global options,
        # since task parameters commonly use the same keys.
    ...
    alias_map_func = globals().get("_alias_map")
    if not callable(alias_map_func):
        def alias_map_func(names: List[str]) -> Dict[str, str]:
            out: Dict[str, str] = {}
            for name in names:
                if not name:
                    continue
    ...
    aliasmap_all = alias_map_func(all_names_for_alias)
    ...
    def run_host(hspec: str):
        ...
        for tname, lines, params in selected:
            ...
            shell_default_lang: Optional[str] = None
            task_env = {}
            for ln in lines:
                stripped = ln.strip()
                if not stripped:
                ...
                try:
                    if stripped.startswith("shell_lang "):
                        shell_default_lang = stripped.split(None, 1)[1].strip()
                        continue

                    if stripped.startswith("shell "):
                        cmd = stripped[6:].strip()

                        # Support inline env assignments: `shell FOO=bar echo "$FOO"`.
                        # The shell itself expands "$FOO" before the per-command env is applied,
                        # so we also add these assignments to the interpolation context.
                        ...
                        try:
                            ...
                            for tok in cmd_tokens:
                                if re.match(r"^[A-Za-z_][A-Za-z0-9_]*=", tok):
                                    k, v = tok.split("=", 1)
                                    inline_env[k] = _interpolate(v, params, task_env)
                                else:
                                    break
                        except Exception:
                            inline_env = {}

                        cmd_env = {**task_env, **inline_env}
                        cmd = _interpolate(cmd, params, cmd_env)
                        lang_hint, remaining_cmd = _parse_lang_bracket(cmd)
                        effective_lang = lang_hint or shell_default_lang
                        if effective_lang:
                            rendered, _ = _render_polyglot_command(
                                effective_lang, remaining_cmd, PFY_ROOT
                            )
                            cmd_to_run = rendered or remaining_cmd
                        else:
                            cmd_to_run = cmd
                        rc = _exec_line_fabric(
                            cmd_to_run, connection, cmd_env, tname, sflag, suser
                        )
                    elif stripped.startswith("makefile "):
                        make_args = _interpolate(stripped[len("makefile ") :].strip(), params, task_env)
                        rc = _exec_line_fabric(
                            f"make {make_args}".strip(),
                            connection,
                            task_env,
                            tname,
                            sflag,
                    ...
                    elif stripped == "build_detect" or stripped.startswith("build_detect "):
                        # Lightweight detection that works locally or over SSH.
                        detect_cmd = (
                            'if [ -f Makefile ] || [ -f makefile ]; then echo "detected Makefile"; '
                            'elif [ -f CMakeLists.txt ]; then echo "detected CMake"; '
                            'elif [ -f package.json ]; then echo "detected Node"; '
                            'elif [ -f Cargo.toml ]; then echo "detected Cargo"; '
                            'else echo "detected unknown"; fi'
                        )
                        rc = _exec_line_fabric(detect_cmd, connection, task_env, tname, sflag, suser)
                    elif stripped == "autobuild" or stripped.startswith("autobuild "):
                        autobuild_cmd = (
                            'echo "[autobuild] starting"; '
                            'if [ -f Makefile ] || [ -f makefile ]; then echo "[autobuild] Makefile detected"; make; '
                            'elif [ -f CMakeLists.txt ]; then echo "[autobuild] CMake detected"; cmake -S . -B build && cmake --build build; '
                            'elif [ -f package.json ]; then echo "[autobuild] Node detected"; npm run build || npm run build:prod || true; '
                            'elif [ -f Cargo.toml ]; then echo "[autobuild] Cargo detected"; cargo build; '
                            'else echo "[autobuild] no supported build files detected"; fi'
                        )
                        rc = _exec_line_fabric(autobuild_cmd, connection, task_env, tname, sflag, suser)
                    else:
                        print(
                            f"[skip] unsupported verb in task '{tname}': {stripped}",
                            file=sys.stderr,
                        )
                        continue
                    if rc != 0:
