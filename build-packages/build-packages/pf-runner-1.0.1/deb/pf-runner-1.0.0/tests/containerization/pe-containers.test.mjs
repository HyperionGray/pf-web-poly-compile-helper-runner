...
async function runTests() {
    ...
    // ==========================================
    console.log('\n--- Section 2: Pfyfile.pe-containers.pf Validation ---');
    const pePfyfilePath = join(projectRoot, 'pf-files', 'mult-exec', 'Pfyfile.pe-containers.pf');

    await tester.test('Pfyfile.pe-containers.pf exists', async () => {
        const exists = await tester.fileExists(pePfyfilePath);
        if (!exists) {
    ...
    await tester.test('Pfyfile has pe-build-all task', async () => {
        const content = await tester.readFile(pePfyfilePath);
