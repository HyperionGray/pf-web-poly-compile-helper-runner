...
class PfTUI:
    ...
    CATEGORY_COLORS = {
        ...
        "tui": "cyan",
        "core": "white",
        # Folder-based categories under pf-files/
        "always-available": "white",
        "auto-api": "cyan",
        "auto-install": "green",
        "containers": "green",
        "debugging-deep": "red",
        "distro-switching": "bright_yellow",
        "enhanced-tasks": "bright_cyan",
        "exploit-writing": "bright_red",
        "gitops": "bright_green",
        "llvm-lifting": "bright_cyan",
        "mult-exec": "magenta",
        "practice": "yellow",
        "vuln-hunting": "bright_red",
        "web-testing": "cyan",
        "web-vulns": "bright_red",
    }
    ...
    def discover_pfy_files(self) -> None:
        ...
        # If the repo keeps Pfyfiles under pf-files/, make sure we scan that too.
        if parent_dir:
            search_dirs.append(os.path.join(parent_dir, "pf-files"))
        seen_files = set()
        ...
        for search_dir in dict.fromkeys(search_dirs):
            ...
            # Prefer recursive discovery under pf-files/ (categorized layout), but keep
            # legacy flat patterns for compatibility.
            patterns = [
                (os.path.join(search_dir, "Pfyfile.pf"), False),
                (os.path.join(search_dir, "Pfyfile.*.pf"), False),
            ]
            if os.path.basename(search_dir) == "pf-files":
                patterns = [(os.path.join(search_dir, "**", "Pfyfile*.pf"), True)]

            for pattern in patterns:
                # pattern can be a tuple (pattern, recursive)
                if isinstance(pattern, tuple):
                    pat, rec = pattern
                else:  # pragma: no cover (defensive)
                    pat, rec = pattern, False

                for filepath in glob.glob(pat, recursive=rec):
                    if filepath in seen_files:
                        continue
                    seen_files.add(filepath)

                    basename = os.path.basename(filepath)
                    # Extract category from folder when under pf-files/<category>/...
                    category = None
                    try:
                        parts = os.path.normpath(filepath).split(os.sep)
                        if "pf-files" in parts:
                            idx = parts.index("pf-files")
                            if idx + 2 < len(parts):  # pf-files/<category>/<...>/<file>
                    ...
                    if basename == "Pfyfile.pf":
                        ...
                        name = file_stem.replace("-", " ").replace("_", " ").title()
                        category = category or file_stem
