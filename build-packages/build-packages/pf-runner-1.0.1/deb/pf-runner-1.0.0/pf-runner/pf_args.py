...
For more help on a specific subcommand:
  pf <subcommand> --help
        ...
        self.parser.add_argument(
            "-f", "--file", help="Specify Pfyfile location (default: pf-files/Pfyfile.pf, fallback: Pfyfile.pf)"
        )
        self.parser.add_argument(
            "--env",
            action="append",
    ...
    def parse_legacy_args(
        ...
        # If no explicit command and we have task args, assume 'run' (unless it's a builtin command)
        if task_args and task_args[0] not in ("list", "help", "run", "prune", "debug-on", "debug-off", "version"):
