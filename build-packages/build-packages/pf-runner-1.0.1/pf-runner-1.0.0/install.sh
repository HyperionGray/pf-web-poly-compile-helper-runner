#!/usr/bin/env bash
# install.sh - Installer for pf-runner (package-first, native fallback; container mode deprecated)
...
show_help() {
    cat << EOF
pf-runner Installation Script (Native / .deb)
...
NOTE:
    Container-based install mode is deprecated; prefer .deb or native.
EOF
}
    ...
    case "$os_type" in
        debian)
            ...
            apt-get install -y python3-dev python3-pip python3-venv build-essential curl git \
                emscripten lfortran clang lld
            ;;
        rhel)
            if command -v dnf >/dev/null 2>&1; then
                dnf install -y python3-devel python3-pip gcc gcc-c++ make curl git || true
                dnf install -y emscripten lfortran clang lld || true
            else
                yum install -y python3-devel python3-pip gcc gcc-c++ make curl git || true
                yum install -y emscripten lfortran clang lld || true
            fi
            ;;
        arch)
            pacman -Sy --noconfirm python python-pip base-devel curl git emscripten lfortran clang lld || true
            ;;
        macos)
            if command -v brew >/dev/null 2>&1; then
                brew install python3 git emscripten lfortran llvm || true
            else
