...
class PfSyntaxChecker:
    ...
    def validate_syntax(self, source: str, source_file: Optional[str] = None) -> ValidationResult:
        ...
        while i < len(lines):
            ...
            # Skip shell line-continuations (`shell ... \` followed by continued lines)
            # These continuation lines may start with `if`/`for`, but they are shell, not DSL.
            ...
            # Skip heredoc bodies (e.g. `shell <<'EOF'` ... `EOF`)
            if in_heredoc:
                if stripped == (heredoc_delim or ""):
                    in_heredoc = False
                    heredoc_delim = None
                i += 1
                continue

            # Skip indented literal shell blocks (e.g. `shell |` + indented script)
            if in_shell_block:
                if not stripped:
                    i += 1
                    continue
                if _indent(line) > shell_block_indent:
                    i += 1
                    continue
                in_shell_block = False
                continue
            ...
            # Detect literal blocks within tasks before attempting to parse DSL keywords
            if current_task and stripped.startswith('shell '):
                cmd = stripped[6:].strip()
                if cmd == '|':
                    in_shell_block = True
                    shell_block_indent = _indent(line)
                    i += 1
                    continue

                m = re.search(r"<<\s*'?(?P<delim>[A-Za-z0-9_]+)'?", cmd)
                if m:
                    in_heredoc = True
                    heredoc_delim = m.group("delim")
                    i += 1
                    continue

                # Backslash-continued multi-line shell commands
            ...
            if stripped.startswith('task '):
                try:
                    ...
                    block_stack.append(('task', line_no, task_name))
                    current_task = task_name
                except Exception as e:
                    ctx_before, ctx_after = self._get_context_lines(lines, line_no)
            ...
            # Track if blocks
                ...
                block_stack.append(('if', line_no, None))
            ...
            # Track for blocks
                ...
                block_stack.append(('for', line_no, None))
